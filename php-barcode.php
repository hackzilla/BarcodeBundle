...
function font_loc()
{
        return dirname(__FILE__)."/"."FreeSansBold.ttf";
}
...
function barcode_save($code, $encoding="ANY", $scale = 2, $mode = "png", $filename = '' )
{
    $bars=barcode_encode($code,$encoding);
    if (!$bars) return;
    if (!$mode) $mode="png";
